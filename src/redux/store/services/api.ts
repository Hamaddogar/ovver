...
import { getBuilderDomain, getCookie } from 'src/auth/context/jwt/utils';
import { RootState } from '../store';
import { endpoints } from 'src/utils/axios';
// Helper
import buildURLWithQueryParamsArray from 'src/utils/buildURLWithQueryParamsArray';
...
export const api = createApi({
  ...
  baseQuery: fetchBaseQuery({
    ...
    prepareHeaders: (headers, { getState }) => {
      ...
      headers.set('x-tenant-id', getBuilderDomain() || '');
      return headers;
    },
  }),
  ...
  endpoints: (builder) => ({
    ...
    // builder details
    getBuilder: builder.query<Object, void>({
      query: () => endpoints.builder.list,
    }),
    getBuilderDetails: builder.query({
      query: (id) => ({
        url: `${endpoints.builder.list}/${id}`,
      }),
    }),
    // upgrade plan
    ...
    getGlobalAnalytics: builder.query({
      query: ({ filterValue }) => {
        const URL = endpoints.analytics.globalAnalytics;
        const queryParameters = [{ key: 'filter', value: filterValue }];
    ...
    getMostSellingProducts: builder.query({
      query: () => endpoints.analytics.mostSellingProduct,
    }),
  }),
});
