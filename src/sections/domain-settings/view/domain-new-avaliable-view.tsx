        ...
        {response?.data?.data?.recommendations ? (
          response?.data?.data?.recommendations.map((domain: any, indx: any) => (
            <Stack
              ...
              key={indx}
              sx={domain.domainName === selected?.domainName ? selectedStyle : normalStyle}
              onClick={() => setSelected(domain)}
            >
              <Typography>{domain.domainName}</Typography>
              {domain?.domainName === selected?.domainName && (
                <Iconify style={{ transition: 'all .4s' }} icon="subway:tick" color="#1BFCB6" />
              )}
            </Stack>
          ))
        ) : (
          <Stack
            ...
            key={1}
            sx={
              response?.data?.data.domainName === selected?.domainName ? selectedStyle : normalStyle
            }
            onClick={() => setSelected(response?.data?.data)}
          >
            <Typography>{response?.data?.data.domainName}</Typography>
            {response.data?.data?.domainName === selected?.domainName && (
              <Iconify style={{ transition: 'all .4s' }} icon="subway:tick" color="#1BFCB6" />
            )}
          </Stack>
        )}
      ...
      <ConfirmDialog
        ...
        content={
          <Box textAlign="center">
            ...
            >
              Your domain is linked Successfully to your website! you can now share and publish your
              website.
            </Typography>
